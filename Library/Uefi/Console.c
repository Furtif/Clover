...
// EFI_CONSOLE_MAX_ATTEMPTS
/// Maximum attempts at a failed method
#define EFI_CONSOLE_MAX_ATTEMPTS 5
...
EfiConOutSetMode (
  ...
  UINTN      Attempts = EFI_CONSOLE_MAX_ATTEMPTS;
  // Check console is valid
  if ((gEfiConOut == NULL) || (gEfiConOut->SetMode == NULL)) {
    return EFI_UNSUPPORTED;
  }
  while (Attempts != 0) {
    Status = gEfiConOut->SetMode(gEfiConOut, ModeNumber);
    if (!EFI_ERROR(Status)) {
      break;
    }
    --Attempts;
